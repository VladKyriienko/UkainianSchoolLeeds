...
import type { AdminUser } from '@/app/(authenticated)/admin/users/actions';
...
export default function UserActionButtons({
  ...
  const handleToggleActive = async () => {
    ...
    if (
      !confirm(`Are you sure you want to ${actionText} user ${user.email}?`)
    ) {
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : `Failed to ${actionText} user`;
      setError(errorMessage);
    } finally {
  ...
  const activationTitle = isActive
  ...
  return (
    <div className="flex items-center gap-1">
      <Button
        ...
        title="Edit user"
      >
        <Edit className="w-4 h-4 text-green-600 dark:text-green-500" />
      </Button>
      ...
      >
        <Mail
          className={`w-4 h-4 ${isSendingEmail ? 'text-muted-foreground' : 'text-yellow-600 dark:text-yellow-500'}`}
        />
      </Button>
      ...
      <Button
        ...
        className={`h-8 w-8 p-0 ${
          isActive ? 'hover:bg-yellow-500/10' : 'hover:bg-green-500/10'
        }`}
        title={activationTitle}
      >
        {isActive ? (
          <UserX
            className={`w-4 h-4 ${isTogglingActive ? 'text-muted-foreground' : 'text-yellow-600 dark:text-yellow-500'}`}
          />
        ) : (
          <UserCheck
            className={`w-4 h-4 ${isTogglingActive ? 'text-muted-foreground' : 'text-green-600 dark:text-green-500'}`}
          />
        )}
      </Button>
      ...
      >
        <Trash2
          className={`w-4 h-4 ${isDeleting ? 'text-muted-foreground' : 'text-destructive'}`}
        />
      </Button>
