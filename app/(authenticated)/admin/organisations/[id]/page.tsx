...
export default async function OrganisationDetailsPage({
  ...
  const membersByRole =
    organisation.organisation_memberships?.reduce(
      (acc, member) => {
    ...
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="mt-8">
        <div className="bg-card p-6 rounded-lg border">
          ...
          organisation.organisation_memberships.length > 0 ? (
            <div className="space-y-4">
              {Object.entries(membersByRole).map(([role, members]) => (
                <div key={role}>
                  <h3 className="text-lg font-medium mb-3 capitalize">
                    {role}s ({members?.length || 0})
                  </h3>
                  <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                    {members?.map((membership, index) => (
                      <div
                        key={index}
                        className="bg-secondary/50 p-4 rounded-lg border"
                      >
                        <div className="flex items-center gap-3 mb-2">
                          <Avatar className="h-8 w-8">
                            <AvatarImage
                              src={membership.user?.avatar_url || undefined}
                              alt={membership.user?.full_name || 'User avatar'}
                            />
                            <AvatarFallback>
